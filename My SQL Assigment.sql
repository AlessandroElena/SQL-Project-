...
/* (a). a.	Fetch the employee number, first name and last name of those employees who 
are working as Sales Rep reporting to employee with employeenumber 1102 (Refer 
employee table) */
...
/* (b) Show the unique productline values containing the word cars at the end from the 
products table. */
...
/* (a) Using a CASE statement, segment customers into three categories based on their 
country */
...
/* (b) b.	Company wants to analyse payment frequency by month. Extract the month name 
from the payment date to count the total number of payments for each month and include 
only those months with a payment count exceeding 20. Sort the results by total number 
of payments in descending order */
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/Customers_Orders /*!40100 DEFAULT CHARACTER SET latin1 */;
...
SELECT A.FullName AS 'Manager Name', B.FullName AS 'Emp Name'
FROM project A
JOIN project B ON A.EmployeeID = B.ManagerID;
...
/* i) Alter the table by adding the primary key and auto increment to Facility_ID 
column. */

Alter table facility
Modify column Facility_ID int auto_increment primary key;

/* ii) Add a new column city after name with data type as varchar which should not 
accept any null values. */
...
select `productlines`.`productLine`, sum(`orderdetails`.`quantityOrdered` * `orderdetails`.`priceEach`)
as total_sales, count(distinct `orders`.`orderNumber`) as number_of_orders
from `productlines`
...
BEGIN
    ...
    WHERE 
        YEAR(`paymentDate`) = input_year AND `country` = input_country
    GROUP BY 
        Year, `country`;
END//
...
rank() over (order by count(`orders`.`orderNumber`) DESC) AS rank_
from `orders`
...
select year(`orderDate`) as Year,
    ...
    concat(format((count(*) - lag(count(*)) over (order by year(`orderDate`), month(`orderDate`))) * 100.0 / nullif(least(lag(count(*)) over (order by year(`orderDate`), month(`orderDate`)), count(*)), 0), 0), '%') AS '% YoYChange'
from `orders`
...
DROP PROCEDURE IF EXISTS error_handling;
...
begin
    ...
    insert into Emp_EH (EmpID, EmpName, EmailAddress) values
...
CALL error_handling(1, 'Sam Smith', 'samsmith@gmail.com');
CALL error_handling(1, 'Sam Smith', 'samsmith@gmail.com');
...
DROP TRIGGER IF EXISTS EmpTrigger;
